<#
  ...
  Writes : html/pages/*.html         student-facing Canvas Page
           ...
           html/teacher/*.html       teacher copy (KEEP UNPUBLISHED)
...
# (--  &rarr;  &#9654;  &#x2b50;) alone so content JSON can use them directly.
...
function Inline([string]$s) {
  ...
  # Single-asterisk emphasis, applied after bold so it cannot eat the doubles.
  # Deliberately conservative:  the run must start with a non-space and may not
...
function VocabList($vocab, [string]$accent, [string]$soft) {
  ...
  foreach ($v in $vocab) {
    [void]$sb.Append('<div style="background:' + $soft + ';border-left:4px solid ' + $accent + ';border-radius:0 5px 5px 0;padding:10px 14px;margin:0 0 8px;">' +
      '<span style="font-weight:700;color:' + $accent + ';">' + (Inline $v.term) + '</span> -- ' + (Inline $v.def) + '</div>')
  }
  return $sb.ToString()
}
...
# A float-right sidebar for cross-language comparisons - "here is what this
# looked like in the language you already know." Warm parchment, so it reads as
# marginalia rather than as instruction (blue) or warning (orange).
#
# Two constraints shape the sizing.  Canvas strips <style> blocks, so there are no
# media queries available:  the width is declared twice, and a browser that
# understands max() takes the second.  Paired with max-width:100% that means a
# narrow phone gets a full-width block instead of a 120px sliver or an overflow.
function Aside($aside) {
  if (-not $aside) { return '' }
  $icon = if ($aside.icon) { $aside.icon } else { '&#9749;' }   # coffee cup
  $bar  = '#8a6a3d'

  $sb = New-Object System.Text.StringBuilder
  [void]$sb.Append('<div style="float:right;width:38%;width:max(250px,38%);max-width:100%;box-sizing:border-box;' +
    'background:#faf7f2;border:1px solid #e5dccd;border-left:4px solid ' + $bar + ';border-radius:0 6px 6px 0;' +
    'padding:12px 16px;margin:2px 0 14px 22px;font-size:14px;line-height:1.5;">')
  [void]$sb.Append('<p style="margin:0 0 7px;font-weight:700;color:' + $bar + ';font-size:13px;">' + $icon + ' ' + (Inline $aside.h) + '</p>')
  foreach ($para in $aside.p) {
    [void]$sb.Append('<p style="margin:0 0 9px;color:#4a4034;">' + (Inline $para) + '</p>')
  }
  if ($aside.bullets) {
    [void]$sb.Append('<ul style="margin:0 0 6px;padding-left:18px;color:#4a4034;">')
    foreach ($b in $aside.bullets) { [void]$sb.Append('<li style="margin:0 0 5px;">' + (Inline $b) + '</li>') }
    [void]$sb.Append('</ul>')
  }
  if ($aside.code) {
    [void]$sb.Append('<pre style="margin:0 0 4px;background:#fffdf9;border:1px solid #e5dccd;border-radius:5px;' +
      'padding:9px 11px;overflow-x:auto;' + $MONO + 'font-size:12px;line-height:1.5;color:#4a4034;white-space:pre;">' +
      (FormatCode $aside.code) + '</pre>')
  }
  [void]$sb.Append('</div>')
  return $sb.ToString()
}
...
function StageTable($standard, $stages) {
  ...
  [void]$sb.Append('<p style="margin:12px 0 0;font-size:14px;color:' + $MUTED + ';"><strong style="color:#1d7a44;">&#11088; Made It Mine is the goal for everyone.</strong> &ldquo;Went Beyond&rdquo; keeps its list open -- the examples are only starting points.  Going somewhere the lesson left open is the whole idea.</p>')
  return $sb.ToString()
}
...
function FileComment($lesson, [string]$kind) {
  return '<!-- ==========================================================' + "`n" +
  '     ' + $lesson.title + '  |  ' + $kind + "`n" +
  '     Generated by build_canvas_html.ps1 - edit the JSON and rebuild.' + "`n" +
...
function Build-Page($lesson, $th) {
  $a = $th.accent
  $sb = New-Object System.Text.StringBuilder
  [void]$sb.Append((Banner $lesson $th ($th.label + ' ' + $lesson.num + '  &middot;  ' + $SERIES)))
  ...
  foreach ($sec in $lesson.howItWorks) {
    ...
    # The aside is emitted before the prose so the prose wraps around it, and
    # the float is closed before any code block so code always gets full width.
    if ($sec.aside) { [void]$sb.Append((Aside $sec.aside)) }
    foreach ($para in $sec.p) { [void]$sb.Append((P $para)) }
    if ($sec.bullets) { [void]$sb.Append((Bullets $sec.bullets)) }
    if ($sec.aside) { [void]$sb.Append((ClearFloat)) }
...
function Build-Assignment($lesson, $th) {
  $a = $th.accent
  $sb = New-Object System.Text.StringBuilder
  [void]$sb.Append((Banner $lesson $th ($th.label + ' ' + $lesson.num + ' Assignment  &middot;  ' + $SERIES)))

  # The scoring note is course-level policy, not a lesson detail, so it comes from
  # course.json.  The default is the non-credit wording the Processing series uses.
  $scoringNote = if ($course.assignmentNote) { $course.assignmentNote } else {
    'The stages below are the standard, and they are the same in every lesson.  They describe **the work** -- and you can revise and re-submit any time.'
  }
  [void]$sb.Append('<div style="background:' + $th.soft + ';border:1px solid ' + $th.edge + ';border-radius:0 0 8px 8px;border-top:none;padding:12px 20px;margin:0 0 8px;font-size:14px;color:' + $MUTED + ';">' +
  ...
  # How to package a submission is toolchain-specific, so it comes from
  # course.json.  Set it to "" to omit the line entirely.
  $submitHint = if ($null -ne $course.submitHint) { $course.submitHint } else {
    'To zip your sketch:  **Sketch &rarr; Show Sketch Folder**, go up one level, then right-click the sketch folder and compress it.'
  }
  if ($submitHint) {
    [void]$sb.Append('<p style="margin:0 0 12px;font-size:14px;color:' + $MUTED + ';">' + (Inline $submitHint) + '</p>')
  }
  [void]$sb.Append('</div>')
...
function Build-Teacher($lesson, $th) {
  ...
  [void]$sb.Append('<div style="background:#b91c1c;color:#ffffff;border-radius:8px 8px 0 0;padding:10px 20px;font-size:13px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;">' +
    '&#9888; Teacher copy -- keep this page unpublished</div>')
  ...
  # Section timings come from the lesson JSON when present, so a course whose
  # lessons run across two meetings can state its own pacing.  The defaults are
  # the original single-period values.
  $tWarm   = if ($lesson.teacher.warmupTime)   { $lesson.teacher.warmupTime }   else { '~5 min' }
  $tDirect = if ($lesson.teacher.directTime)   { $lesson.teacher.directTime }   else { '~10 min' }
  $tGuided = if ($lesson.teacher.guidedTime)   { $lesson.teacher.guidedTime }   else { '~20 min' }
  $tWrap   = if ($lesson.teacher.wrapupTime)   { $lesson.teacher.wrapupTime }   else { '~5 min' }

  [void]$sb.Append((H2 ('Warm-up (' + $tWarm + ')') $a))
  [void]$sb.Append((P $lesson.warmup))

  [void]$sb.Append((H2 ('Direct instruction (' + $tDirect + ')') $a))
  ...
  [void]$sb.Append((H2 ('Guided activity (' + $tGuided + ')') $a))
  ...
  [void]$sb.Append((H2 ('Wrap-up (' + $tWrap + ')') $a))
  ...
  return Shell (FileComment $lesson 'TEACHER COPY - KEEP UNPUBLISHED') $sb.ToString()
}
...
$index.Add('**Leave these HTML files alone** -- edit the JSON and re-run the build.')
$index.Add('')
$index.Add('Each file is a **body fragment**:  open it, copy everything, and paste it into the Canvas HTML editor')
$index.Add('(the `</>` button at the bottom-right of the Rich Content Editor).  Every style is inline and every')
$index.Add('tag is one Canvas allows, so the markup survives the paste exactly as written.')
$index.Add('')
...
$index.Add('')
$index.Add('```bash')
$index.Add('grep -rn ''"flintUrl"'' ../content/          # what is set')
$index.Add('grep -rln PASTE_FLINT pages assignments   # what is still waiting')
$index.Add('```')
$index.Add('')
...
# --- course-level content, loaded before the lessons so the banners can use it ---
...
# The short name that appears in every lesson banner.  Falls back to the course
# title, so course.json does not have to declare it.
$SERIES = if ($course.seriesName) { $course.seriesName } else { $course.title }
...
# --- front page ---   ($course was loaded before the lesson loop, above)
